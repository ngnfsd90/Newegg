    ...
    <>
      <div className="relative">
        <button
          ...
          className={`text-white flex items-center space-x-1 sm:space-x-2 text-xs sm:text-sm rounded-full transition-all ${darkMode ? "hover:bg-[#3a3a3a] hover:px-2 hover:py-1 sm:hover:px-3 sm:hover:py-1.5" : "hover:bg-[#0040a0] hover:px-2 hover:py-1 sm:hover:px-3 sm:hover:py-1.5"}`}
        >
          <FaMapMarkerAlt className="text-white text-sm sm:text-base md:text-xl flex-shrink-0" />
          <span className="hidden sm:inline text-white">{selectedCountry ? `${selectedCountry} تحویل داده شود` : 'انتخاب موقعیت شما'}</span>
          <span className="sm:hidden text-white text-xs">موقعیت</span>
        </button>
      </div>
      {isOpen && (
        <div 
          className={`fixed ${darkMode ? "bg-[#121212] text-white" : "bg-white text-black"} border rounded-lg p-4 sm:p-6 md:p-10 w-[90vw] sm:w-80 md:w-96 rtl shadow-xl z-[60] max-h-[calc(100vh-80px)] overflow-y-auto`}
        ...
        >
          ...
          <div className="flex flex-col items-center">
            <button
              className={`py-2 px-6 rounded-full ${isDoneEnabled ? 'bg-[#FA9D29] text-white' : 'bg-gray-300 text-gray-600'}`}
