...
export default function Categories({ title, imageUrl, linkText, linkUrl }) {
  return (
    ...
    >
      <div className="absolute top-4 left-4 transform -translate-y-1/2 text-white text-3xl font-bold">
        {title}
      </div>

      <a
        href={linkUrl}
        className="absolute bottom-4 left-4 text-white p-2 bg-black rounded-md hover:bg-gray-700"
      >
        {linkText}
      </a>
